...
case $opcao in
    ...
    3)
        ...
        if [ "$VALIDADO" = true ]; then
            ...
            monkey -p com.dts.freefireth -c android.intent.category.LAUNCHER 1 > /dev/null 2>&1 || monkey -p com.dts.freefiremax -c android.intent.category.LAUNCHER 1 > /dev/null 2>&1
        else
