...
case $opcao in
    ...
    4)
        ...
        echo -e "[3] Otimizar Windows/System Buffer"
        echo -e "[4] Limpeza Geral de Cache"
        read -p "Escolha a função: " sub_op

        if [ "$sub_op" == "1" ]; then
            echo -e "${GREEN}Configurando 120 FPS...${RESET}"
            setprop persist.sys.NV_FPSLIMIT 120; setprop ro.vendor.display.default_fps 120; setprop debug.egl.swapinterval 0
            setprop debug.composition.type gpu; setprop ro.display.prefer_high_fps 1; setprop sys.fps.limit 120
            setprop ro.vendor.display.ad.sdr_ve 1; setprop hwui.fps_divisor 1; setprop debug.sf.max_fps 120; setprop ro.sf.lcd_density 480
        elif [ "$sub_op" == "2" ]; then
            echo -e "${PURPLE}Otimizando códigos internos e Mira...${RESET}"
            echo -e "${YELLOW}Ativando X e Y...${RESET}"
            ...
            # Comandos adicionais de estabilização de mira
            cmd package bg-dexopt-job >/dev/null 2>&1
            setprop ro.input.sens.x 1
            setprop ro.input.sens.y 1
            setprop windowsmgr.max_events_per_sec 150
            setprop windowsmgr.display_width 1080
            
            setprop ro.min_pointer_dur 12
            setprop ro.max.fling_velocity 3500
            setprop touch.pressure.scale 0.0001
            setprop windowsmgr.max_events_per_sec 100
        elif [ "$sub_op" == "3" ]; then
            echo -e "${BLUE}Otimizando Windows Buffers...${RESET}"
            ...
            echo -e "${GREEN}Limpeza executada com sucesso!${RESET}"
            cmd package bg-dexopt-job >/dev/null 2>&1
        fi
        
        echo -e "\n${RED}${BOLD}🚨 INJETANDO AIMBOT 🚨${RESET}"
        
        # Otimizando metadados para melhor fluidez e redenrização
        setprop debug.render.quality 1
        setprop ro.config.hw_video_render 1
        setprop hwui.render_dirty_regions false
        setprop persist.sys.ui.hw 1
        setprop ro.opengles.version 196610
        
        setprop ro.config.hw_quickpoweron true > /dev/null 2>&1
        setprop ro.sys.fw.bg_apps_limit 10 > /dev/null 2>&1
        setprop ro.config.hw_ramsize 8192 > /dev/null 2>&1
